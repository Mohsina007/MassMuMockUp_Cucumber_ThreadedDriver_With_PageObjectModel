...
public class PageBase extends ScriptBase {
    ...
    public WebElement waitForElement(final By locator, int timeToWaitInSec) {
        Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(timeToWaitInSec, TimeUnit.SECONDS).pollingEvery(100, TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class);
    ...
    //highlight when clickwith delayfor
    ...
    //js set attribute if send key or anyattribute normally doesn't work then it is useful
